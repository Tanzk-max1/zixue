...
p2  low-level const 可以更改v2导致p2更改

p3  both low-level and top-level const 左边底层，可以被i更改，可是右边不行，一早被v1赋值初始化了

```
顶层 const：表示指针本身是一个常量，即指针的值（地址）是不可变的。这意味着不能将指针重新指向其他对象。声明为 *const p3 的指针 p3 是一个顶层 const。

底层 const：表示指针所指向的对象是一个常量，即通过指针不能修改所指对象的值。声明为 *const p3 的指针所指向的对象，即 *p3 是一个底层 const。

因为同时取值的话，右边是底层，左边的话是因为
```
...
typedef wages base, *p; // base 是double同义词  , p 是 double*的同义词
```
...
decltype(ci) x = 0;     // x 类型 const int
...
`decltype((var))`的结果永远是引用（双层括号永远都是引用，那就是别名而已），而`decltype(var)`的结果只有当`var`本身是一个引用时才会是引用。

![image-20240417102633260](C:\Users\15525\AppData\Roaming\Typora\typora-user-images\image-20240417102633260.png)

得到的结果分别是3，4，3，3，-》3，4，4，3-》4，4，4，4，因为给d+1时候也给a+1了
